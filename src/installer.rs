...
pub fn install() -> Result<(), Error> {
    let rustup = env::split_paths(&env::var_os("PATH").unwrap_or_default())
        .map(|p| p.join("rustup").with_extension(env::consts::EXE_EXTENSION))
        .find(|p| p.exists());
    let rustup = match rustup {
        Some(path) => path,
        None => {
            bail!("failed to find an installation of `rustup` in `PATH`, \
    ...
    if destination.exists() {
        if !env::args().any(|arg| arg == "-f") {
            bail!("existing wasm-pack installation found at `{}`, pass `-f` to \
                   force installation over this file, otherwise aborting \
                   installation now", destination.display());
        }
    }
